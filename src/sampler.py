...
# import logging
# logging.basicConfig(filename='logs/sampler.log',level=logging.DEBUG)
    ...
    def run_chain(self):
        ...
        for i in range(1, self.params["num_samples"] + 1):
            ...
            for k in range(num_clusters):
                print("MCMC Chain: ", i, "Cluster loop", k)
            ...
            for j in range(self.n):
                print("MCMC Chain: ", i, "Dataset: ", j)
                num_pts_cluster_tmp = self.chain["assignments"]
                cluster_assigned = self.chain["assignments"][i - 1, j]
                num_pts_cluster_tmp[cluster_assigned] = num_pts_clusters[cluster_assigned]
                
                

                # probability for each existing k cluster -> gives a vector of probabilities
                p_old_cluster = norm(
                    self.chain["mu"][i], self.chain["sigma"][i]
                ).pdf(self.data[j])
                mu_new = normal(
                ...
                sigma_new = 1 / np.sqrt(
                    gamma(
                        shape=self.hyperparam["alpha_0"], scale=self.hyperparam["beta_0"]
                    )
                )
                ...
                if cluster_pick == 0:
                    ...
                    self.chain["assignments"][i, j] = num_clusters
            ...
            weights_new = dirichlet(
                alpha=self.params["alpha"] + list(num_pts_clusters.values())
            )
